# SSL 인증서 우회 - JetBot 의존성 설치 (PowerShell)
# PowerShell에서 실행: .\install_ssl_bypass.ps1

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "SSL 인증서 우회 - JetBot 의존성 설치" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
# pip 설정 파일 생성 (SSL 인증서 우회)
Write-Host "🔧 pip 설정 파일 생성 중..." -ForegroundColor Yellow
...
$pipConfig | Out-File -FilePath "$pipDir\pip.conf" -Encoding UTF8
Write-Host "✅ pip 설정 파일 생성 완료" -ForegroundColor Green
Write-Host ""
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ OpenCV 설치 실패, 대체 방법을 시도합니다..." -ForegroundColor Yellow
